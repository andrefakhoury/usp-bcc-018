/**
 * =========================================
 * + Atividade 2.3 - Computação Gráfica    +
...
int main() {
	...
	// Prepare vertices
	const int QT_VERTICES = 100;
	const double radius = 0.3;

	Point2D vertices[QT_VERTICES];
	for (int i = 0; i < QT_VERTICES; i++) {
		double cur_theta = 2 * M_PI / QT_VERTICES * i;
		vertices[i].x = cos(cur_theta) * radius;
		vertices[i].y = sin(cur_theta) * radius; 
	}
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glDrawArrays(GL_TRIANGLE_FAN, 0, QT_VERTICES);
