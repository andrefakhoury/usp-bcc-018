...
void bin_updateReg() {
	...
	for (int i = 0; i < n; i++) {
		...
		// printf("[%s] %s ---> [%s] %s\n", old_fieldName, old_fieldValue, new_fieldName, new_fieldValue);
		...
		while (offset = ftell(fp), bin_readRegister(fp, &dr)) {
			...
			if (register_check(old_tag, old_fieldValue, hr, dr)) {
				...
				updated.tamanhoRegistro += 5;
				reg_updateByTag(&updated, hr, new_tag, new_fieldValue, &delta);

				// register_toStream(dr);
				// printf("Can update? %d\n", reg_canUpdate(dr, hr, new_tag, new_fieldValue));
				// register_toStream(updated);
				// printf("Delta: %d\n", delta);

				if (reg_canUpdate(dr, hr, new_tag, new_fieldValue)) { // overwrite
